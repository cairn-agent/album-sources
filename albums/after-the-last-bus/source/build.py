"""Compile Shelter Engine, render scores, master WAVs, and validate loudness.
...
def run(args):
    ...
    return p
def loudness(path,target=-15.5):
    p=run(['ffmpeg','-hide_banner','-i',path,'-af',f'loudnorm=I={target}:TP=-1.4:LRA=18:print_format=json','-f','null','-'])
...
def render(s):
    ...
    if not raw.exists() or not marker.exists() or marker.read_text()!=fingerprint:
        p=run([ROOT/'source'/'shelter-engine',ROOT/'scores'/f'{stem}.tsv',raw])
        (ROOT/'work'/f'{stem}-render.json').write_text(p.stdout)
        marker.write_text(fingerprint)
    target=-21.0 if s['number']==5 else -15.5
    measure=loudness(raw,target)
    ...
    filt=(f'loudnorm=I={target}:TP=-1.4:LRA=18:linear=true:'
    ...
    if s['number']==5:
        # Preserve the quiet piece's transients and exact sample timing with
        # fixed gain. Its -21 LUFS target leaves room below the peak ceiling.
        gain_db=min(target-float(measure['input_i']),-1.4-float(measure['input_tp']))
        filt=f'volume={gain_db:.6f}dB'
    # Normalize timestamps and preserve the requested effects-tail duration.
    filt+=f',asetpts=PTS-STARTPTS,apad=pad_dur=0.25,atrim=duration={trim_to:.6f},afade=t=out:st={max(0,trim_to-.2)}:d=0.2'
    run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',raw,'-t',f'{trim_to:.6f}','-af',filt,'-ar','44100','-c:a','pcm_s24le',dest])
    master=loudness(dest,target)
    ...
    assert abs(seconds-trim_to)<.05,(stem,seconds,trim_to)
    assert -30<float(master['input_i'])<-8
    assert float(master['input_tp'])<=-.7
    check.write_text(json.dumps(dict(title=s['title'],stem=stem,seconds=seconds,bpm=s['bpm'],key=s['key'],renderFingerprint=fingerprint,masteringRevision=2,targetLufs=target,masteringMethod='fixed gain' if s['number']==5 else 'two-pass loudnorm',trimmedTailSeconds=round(nominal-seconds,4),rawLoudness=measure,masterLoudness=master,format='44.1 kHz / 24-bit PCM stereo',renderWallSeconds=round(time.monotonic()-start,2)),indent=2)+'\n')
...
def main():
    ...
    run(['clang++','-O3','-std=c++17',ROOT/'source'/'engine.cpp','-o',ROOT/'source'/'shelter-engine'])
