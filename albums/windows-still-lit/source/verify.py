...
import hashlib, json, math, struct, subprocess, sys, zipfile
import numpy as np
...
def pcm_stats(path,sections,bpm):
    ...
    runlen=longest=0
    # Deliberately fading outros are outside the interior-dropout window.
    outro=next((s['bar']*4*60/bpm for s in sections if s['name']=='outro'),len(ar)*.1-2)
    for value in ar[20:round(outro*10)]:
    ...
    out=dict(samplePeakDb=round(20*math.log10(peak),3),dcMean=sm.tolist(),firstAudibleSeconds=first,lastAudibleSeconds=last,longestInteriorSilenceSeconds=round(longest*.1,2),stereoCorrelation=round(cross/math.sqrt(ss[0]*ss[1]),4),sectionRmsDb=ranges,interiorWindowEndSeconds=round(outro,3))
...
def main():
    ...
    package()
    print('All audio checks passed.',flush=True)

def package():
    # Portable listening edition excludes the redundant complete-album MP3.
    files=[ROOT/p for p in ['album.json','album.m3u8','index.html','tracklist.txt','LINER-NOTES.md','README.md','LYRICS.md']]
    if (ROOT/f'{NAME}-sampler.mp3').exists():files.append(ROOT/f'{NAME}-sampler.mp3')
    files += sorted((ROOT/'mp3').glob('*.mp3'))+sorted((ROOT/'art').glob('*'))+sorted((ROOT/'lyrics').glob('*.lrc'))
    files += [p for p in (ROOT/'lyrics/site-export').rglob('*') if p.is_file()]
    with zipfile.ZipFile(ROOT/f'{NAME}-listening-edition.zip','w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
        for p in files:z.write(p,p.relative_to(ROOT))
    files=[ROOT/'LINER-NOTES.md',ROOT/'README.md',ROOT/'LYRICS.md',ROOT/'tracklist.txt',ROOT/'album.json']
    for folder in ['source','midi','scores','lyrics','art','checks']:
        files += [p for p in (ROOT/folder).rglob('*') if p.is_file() and '__pycache__' not in p.parts and p.name!='window-engine']
    ...
    # Include lyric validation receipts after the optional schema/player checks.
    if (ROOT/'lyrics/site-export/manifest.json').exists():
        with zipfile.ZipFile(ROOT/f'{NAME}-timed-lyrics.zip','w',zipfile.ZIP_DEFLATED,compresslevel=7) as z:
            for p in sorted((ROOT/'lyrics/site-export').rglob('*')):
                if p.is_file():z.write(p,p.relative_to(ROOT/'lyrics/site-export'))
            for p in sorted((ROOT/'checks').glob('lyrics-*.json')):z.write(p,p.relative_to(ROOT))
        with zipfile.ZipFile(ROOT/f'{NAME}-timed-lyrics.zip') as z:assert z.testzip() is None
    print('Listening, studio-source and timed-lyric archives verified.',flush=True)
